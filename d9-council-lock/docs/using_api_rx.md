...
- [Using `ApiRx`](#using-apirx)
  ...
  - [`lock_proposals`](#lock_proposals-1)
    - [Description](#description-4)
    - [How to Call](#how-to-call-3)
  - [`lock_referendums`](#lock_referendums-1)
    - [Description](#description-5)
    - [How to Call](#how-to-call-4)
  - [`concluded_lock_referendums`](#concluded_lock_referendums-1)
    - [Description](#description-6)
    - [How to Call by Session and Account ID](#how-to-call-by-session-and-account-id-1)
  ...
  - [`propose_unlock`](#propose_unlock)
    ...
    - [How to Call](#how-to-call-7)
  - [`vote_on_proposal`](#vote_on_proposal)
    - [Description](#description-10)
    - [How to Call](#how-to-call-8)
...
async function getProposalFee() {
  ...
  const fee = await firstValueFrom(api.query.d9CouncilLock.proposalFee());
...
async function getLockProposal(accountId) {
  ...
  const proposal = await firstValueFrom(api.query.d9CouncilLock.lockProposals(accountId));
...
async function getLockReferendum(accountId) {
  ...
  const referendum = await firstValueFrom(api.query.d9CouncilLock.lockReferendums(accountId));
...
async function getConcludedLockReferendum(sessionIndex, accountId) {
  ...
  const referendum = await firstValueFrom(api.query.d9CouncilLock.concludedLockReferendums(sessionIndex, accountId));
...
async function getAllConcludedReferendums(api) {
  const entries = await firstValueFrom(api.query.d9CouncilLock.concludedLockReferendums.entries());
...
async function getLockedAccount(accountId) {
  ...
  const lockState = await firstValueFrom(api.query.d9CouncilLock.lockedAccounts(accountId));
...
async function proposeLock(signer, accountToLock, proposalFee) {
  ...
  const unsub = api.tx.d9CouncilLock.proposeLock(accountToLock, proposalFee)
    .signAndSend(signer)
    .subscribe((result) => {
...
async function proposeUnlock(signer, accountToUnlock, proposalFee) {
  ...
  const unsub = api.tx.d9CouncilLock.proposeUnlock(accountToUnlock, proposalFee)
    .signAndSend(signer)
    .subscribe((result) => {
      if (result.status.isInBlock) {
        console.log(`Included at block hash ${result.status.asInBlock}`);
...
async function voteOnProposal(signer, lockCandidate, assentOnDecision) {
  ...
  const unsub = api.tx.d9CouncilLock.voteOnProposal(lockCandidate, assentOnDecision)
    .signAndSend(signer)
    .subscribe((result) => {
      if (result.status.isInBlock) {
        console.log(`Included at block hash ${result.status.asInBlock}`);
